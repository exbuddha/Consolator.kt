...
internal sealed interface ImplicitScope<in S> {
    ...
    companion object : DefaultScope, Implication<Any?> {
        ...
        override fun <S : AnyKCallable> AnyKCallable.intercept(vararg args: Any?): S = this as S
    }
}
...
internal fun <T, S : () -> T> select(vararg pointer: S?, predicate: (S?) -> Boolean = ::trueWhenNotNull, condition: (T?) -> Boolean = ::trueWhenNotNull): T? =
    pointer.select(Array<out S?>::find, predicate, condition)

internal fun <T, S : () -> T> selectLast(vararg pointer: S?, predicate: (S?) -> Boolean = ::trueWhenNotNull, condition: (T?) -> Boolean = ::trueWhenNotNull): T? =
