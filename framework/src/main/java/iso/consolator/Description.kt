...
fun <I : Thread, T, R> I.from(lic: KCallable<T>, block: () -> R): R = block()

context(_: LifecycleOwner)
fun <I : Thread, R> I.from(lic: Tag, block: () -> R): R = block()

context(_: LifecycleOwner)
fun <I : Thread, R> I.from(lic: TagType, block: () -> R): R = block()

context(_: LifecycleOwner)
fun <I : Thread, S : ProcessScope, T, R> I.from(lic: KCallable<T>, scope: S, thru: S.() -> R): R = scope.thru()

context(_: LifecycleOwner)
fun <I : Thread, S : ProcessScope, R> I.from(lic: Tag, scope: S, thru: S.() -> R): R = scope.thru()

context(_: LifecycleOwner)
fun <I : Thread, S : ProcessScope, R> I.from(lic: TagType, scope: S, thru: S.() -> R): R = scope.thru()

context(_: LifecycleOwner)
fun <I : Runnable, T, R> I.from(lic: KCallable<T>, block: () -> R): R = block()

context(_: LifecycleOwner)
fun <I : Runnable, R> I.from(lic: Tag, block: () -> R): R = block()

context(_: LifecycleOwner)
fun <I : Runnable, R> I.from(lic: TagType, block: () -> R): R = block()

context(_: LifecycleOwner)
fun <I : Runnable, S : ProcessScope, T, R> I.from(lic: KCallable<T>, scope: S, thru: S.() -> R): R = scope.thru()

context(_: LifecycleOwner)
fun <I : Runnable, S : ProcessScope, R> I.from(lic: Tag, scope: S, thru: S.() -> R): R = scope.thru()

context(_: LifecycleOwner)
fun <I : Runnable, S : ProcessScope, R> I.from(lic: TagType, scope: S, thru: S.() -> R): R = scope.thru()
...
internal fun filterIsEvent(it: Any): Boolean = it.typeIs<Event, _>()
...
inline infix fun Step?.givenSuspended(crossinline predicate: Predicate): UnitSuspendFunction? =
    givenSuspended(predicate) {}

@Suppress("NOTHING_TO_INLINE")
inline infix fun Step?.unlessSuspended(noinline predicate: Predicate): UnitSuspendFunction? =
...
inline infix fun AnyFunction?.given(crossinline predicate: Predicate): AnyFunction? =
    given(predicate) {}

@Suppress("NOTHING_TO_INLINE")
inline infix fun AnyFunction?.unless(noinline predicate: Predicate): AnyFunction? =
    unless(predicate) {}
...
inline infix fun AnyToAnyFunction?.given(crossinline predicate: Predicate): AnyToAnyFunction? =
    given(predicate) {}

@Suppress("NOTHING_TO_INLINE")
inline infix fun AnyToAnyFunction?.unless(noinline predicate: Predicate): AnyToAnyFunction? = letResult {
    unless(predicate) {} }
...
internal fun <R> KCallable<R>.mapToTypedArray(args: KParameterMap): AnyArray =
