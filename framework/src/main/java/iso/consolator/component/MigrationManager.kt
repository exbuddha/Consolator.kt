...
import iso.consolator.CoroutineStep
...
abstract class MigrationManager : Resolver {
    interface Transactor : Table {
        val startMigration: CoroutineStep
    ...
    private fun Byte.toPercentage(): Byte =
        times(100).div(Byte.MAX_VALUE).toByte()

    internal fun expire(): Unit = ::applicationMigrationManager.expire()
}
